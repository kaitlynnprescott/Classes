    ...
    (lambda (exp tenv)
      (cases expression exp
        ...
        (unpair-exp (id1 id2 exp1 body)
                    (let ((t-pair (type-of exp1 tenv)))
                      (cases type t-pair
                        (pair-type (t1 t2) (type-of body (extend-tenv id1 t1 (extend-tenv id2 t2 tenv))))
                        (else (write "error!")))))
        ...
        (node-exp (data left right)
                   (let ((t1 (type-of data tenv))
                         (t2 (type-of left tenv))
                         (t3 (type-of right tenv)))
                    (cases type t2
                      (tree-type (type)
                                 (check-equal-type! type t1 data))
                      (else (write "error!")))
                     (cases type t3
                       (tree-type (type)
                                  (check-equal-type! type t1 data)
